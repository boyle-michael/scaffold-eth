...
import "./App.css";
import {Row, Col, Button, Menu, Alert, List, Input, Progress, Space} from "antd";
...
function App(props) {
    ...
    console.log("📟 withdraw events:", stakeEvents)
    ...
    */
    ...
    if (localChainId && selectedChainId && localChainId != selectedChainId) {
        networkDisplay = (
            <div style={{zIndex: 2, position: 'absolute', right: 0, top: 60, padding: 16}}>
                <Alert
                    ...
                    description={(
                        <div>
                            You have <b>{NETWORK(selectedChainId).name}</b> selected and you need to be
                            on <b>{NETWORK(localChainId).name}</b>.
                        </div>
                    )}
                    type="error"
                    closable={false}
        ...
        networkDisplay = (
            <div
                style={{zIndex: 2, position: 'absolute', right: 154, top: 28, padding: 16, color: targetNetwork.color}}>
                {targetNetwork.name}
            </div>
        )
    }
    ...
    return (
        <div className="App" style={{backgroundColor: 'white'}}>
            ...
            <BrowserRouter>
                ...
                <Route exact path="/">
                        ...
                        <Col span={6}>
                            <div style={{padding: 8}}>
                                <Progress
                                    ...
                                    //     to: '#87d068',
                                    // }}
                                    percent={(stakerContractBalance < exampleExternalContractBalance)
                                        ? exampleExternalContractBalance / threshold * 100
                                        : stakerContractBalance / threshold * 100}
                                />
                            </div>
                        </Col>
                        <Col span={6}></Col>
                    ...
                    <Row>
                        <Col span={24}>
                            <Space>
                                <Button type={balanceStaked ? "success" : "primary"} shape={'round'} onClick={() => {
                                ...
                                <Button type={balanceStaked ? "success" : "primary"} shape={'round'} onClick={() => {
                                    tx(writeContracts.Staker.stake({value: parseEther("1")}))
                                }}>🥩 Stake 1 ether!</Button>
                            </Space>
                        </Col>
                    </Row>
            ...
            <div style={{marginTop: 32, opacity: 0.5}}>Created by <Address
                value={"0x79522998AF5Ce9895842ab7a6210b5E224C089d2"}
                ensProvider={mainnetProvider}
                fontSize={16}
            /></div>
